...
- **单边尺寸硬上限(v0.4.2 实测,最重要的一条)**:surface 捕获单边超过 ~16384 设备 px
  ...
  **必须捕获前约束**:`geom.MAX_CAPTURE_DIM = 16000`(安全边距),整幅/兜底/元素
  条带三条路径统一遵守。
- **超限优先降尺度而非分段**(v0.4.3,用户决策「能单张就单张」):`MIN_FULL_SCALE=0.4`
  ——单边超限先降仿真分辨率整幅(17756px@dpr1 → 0.9× 单张,肉眼几乎无差),
  0.4× 下限内可单张覆盖 ~40000 CSS px;更低清晰度对识图/人眼都无意义,才落分段
  (每段 ≤ maxPartDeviceH=12000,天然安全)。
