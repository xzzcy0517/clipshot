...
- **v0.2.1 虚拟滚动 SPA(飞书文档类)实测教训**:
  ...
  - **v0.7.4:内部滚动容器页的整幅仿真高度 = 容器 scrollHeight + 容器盒顶部偏移
  (scrollerTop)+ 余量**,不能只用 scrollHeight——容器上方有头部栏时,容器可视高
  = 视口 − 头部,底部一截内容永远落在视口外(看板页实测「底部少一截」根因);
  FIXED_CONTAINER 判定须对照容器内容高而非仿真高。
- **v0.4.5:虚拟列表的块高估算随视口配置变化**——探测/滚动/拍摄必须保持**同一
    仿真视口贯穿全程**;每段「清除→重建」仿真(真实视口↔拍摄视口反复横跳)会让
    布局持续涌动、固定 scrollTop 下内容漂移,相邻段重叠(用户实测 15 段大量重复)。
    滚动预热也必须在拍摄视口下进行,高度才收敛到拍摄态;同配置 resize 要去重跳过。
...
- **v0.10.6:内部容器页的页头/侧栏是流内元素,hideFixed 抓不到**(飞书 wiki
  navigation-bar,64px,与样本 2 的 scrollerTop 互证;用户实测:手动删除后
  分段不再重复)。它让每个分段顶部重复一条页头、且容器实高比仿真视口少 64px。
  对策:`hideOutOfScrollerChrome` 沿容器祖先链 display:none 掉「不含容器」的
  兄弟子树(随 HIDE_FIXED fullPage 启用,仅整页捕获;元素/框选禁用,防回流
  使 rect 失效),隐藏后容器顶到 0、占满仿真视口,v0.7.4 的 scrollerTop 补偿
  自动退化。**自动机制的实测结论待用户验证后回填,标 pending。**
- **单边尺寸硬上限(v0.4.2 实测,最重要的一条)**:surface 捕获单边超过 ~16384 设备 px
  (GPU 纹理上限)时**不报错**——输出尺寸正确、宽高比对账通过,但超限尾部内容
  「回绕」成首屏区域的复制(17756px@dpr1 实测)。任何几何校验都防不住这种损坏,
  **必须捕获前约束**:`geom.MAX_CAPTURE_DIM = 16000`(安全边距),整幅/兜底/元素
  条带三条路径统一遵守。
- **禁止分数 deviceScaleFactor(v0.4.4,同机同文档 A/B 实测,推翻 v0.4.3 的降尺度方案)**:
  1.0× 时 17756px 页中段内容全部正确(仅超 16384 的尾部回绕);0.9× 时中段大量
  「截取了一半」、后半程花屏乱序——**超大视口 + 非整数栅格缩放会让 Chrome 栅格化
  从后半程崩坏**,且宽高比对账完全发现不了。结论:仿真只允许原生 dpr;原生装不下
  (单边 >16000 或面积 >60M)一律分段,每段视口高度正常(≤maxPartDeviceH=12000),
  虚拟列表与栅格化都在安全区;人看的「单张」由预览端合成/分卷承担(下载仍是单文件)。
- preview 画布合成上限 32767 单边设备 px → 超出走分卷(每卷 ≤30000),分卷失败才逐段列表。
