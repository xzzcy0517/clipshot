...
1. **接管显式化**:面板新增「Agent 接管」总开关;接管中扩展角标变色 +
   页面右上角常亮小徽标「🤖 Agent 控制中 · Esc 交还」;`ttlSec` 默认 120s,
   每次 act 续期,超时自动释放并广播事件;Esc/点徽标一键夺回;
2. **危险动作拦截**:目标文本匹配 `支付|付款|转账|删除|提现|解绑|退出登录`
   或 `input[type=password]` → 返回 `CONFIRM_REQUIRED`,须 act 带 `"confirm":true`
   才执行(黑名单词表在设置页可编辑);
3. **域名黑名单**(设置页):命中则 control 直接拒绝(默认建议填网银/邮箱域);
4. **步数与频率**:每会话 act 上限 40 步、最短间隔 400ms(防模型打转);
5. 沿用既有防线:127.0.0.1 + JSON 预检;接管开关不勾,整个面物理不存在。
